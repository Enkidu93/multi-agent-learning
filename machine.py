        ...
        if message.seq_num > self.last_received_from.get(message.sender,-1):
            if message.proposal == "proposal":
                ...
                for a in self.world.agents:
                    ...
                    a_x = a_abs_pos[0]
                    a_y = a_abs_pos[1]
                    if a.name != message.sender and sqrt((a_x - sender_x)**2 + (a_y - sender_y)**2)<=1 and ((atan2((a_y-sender_y),(a_x-sender_x)))<=((sender_theta + pi/8)%(2*pi)) or (atan2((a_y-sender_y),(a_x-sender_x)))>=((sender_theta - pi/8)%2*pi)):
