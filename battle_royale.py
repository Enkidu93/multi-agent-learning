...
class BattleRoyale(World):
    ...
    def translateAbsoluteState(self,agent:NetworkAgent):
        ...
        if agent.use_ping:
            ping_to_foe1 = agent.get_ping_time_to(foe1)
            ping_to_foe2 = agent.get_ping_time_to(foe2)
            return [round(wall_r,0),round(wall_dtheta,0),round(foe1_r,0),round(foe1_dtheta,0),round(foe1_reltheta,0),round(foe2_r,0),round(foe2_dtheta,0),round(foe2_reltheta,0),round((ping_to_foe1 + ping_to_foe2)/15, 0)]#,round(ping_to_foe2/30, 0)]
